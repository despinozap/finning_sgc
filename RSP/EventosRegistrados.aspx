<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="EventosRegistrados.aspx.cs" Inherits="NCCSAN.RSP.EventosRegistrados" %>
                ...
                <table align="center" class="tableControls" style="width: 100%">
                    <tr>
                        ...
                        <td class="tdUnique">
                            ...
                            <asp:DropDownList ID="ddlCliente" runat="server" DataSourceID="SDSCliente" DataTextField="nombre"
                                DataValueField="nombre" OnDataBound="ddlCliente_DataBound" AutoPostBack="True"
                                Width="300">
                            </asp:DropDownList>
                            <asp:SqlDataSource ID="SDSCliente" runat="server" ConnectionString="<%$ ConnectionStrings:NCCSANConnectionString %>"
                                SelectCommand="SELECT [nombre] FROM [Cliente]">
                            </asp:SqlDataSource>
                        </td>
                        <td class="tdUnique">
                    ...
                    <tr>
                        <td class="tdUnique" colspan="4" align="center">
                    ...
                    <td class="tdUnique">
                        <asp:GridView ID="gvEventos" runat="server" AutoGenerateColumns="False" DataKeyNames="codigo"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="nombre_tipoequipo" HeaderText="Equipo" 
                                    SortExpression="nombre_tipoequipo">
                                <ItemStyle HorizontalAlign="Center" />
                                </asp:BoundField>
                                <asp:BoundField DataField="modelo_equipo" HeaderText="Modelo" 
                                    SortExpression="modelo_equipo">
                                <ItemStyle HorizontalAlign="Center" />
                                </asp:BoundField>
                                <asp:BoundField DataField="nombre_componente" HeaderText="Componente" 
                                    SortExpression="nombre_componente">
                                    <ItemStyle HorizontalAlign="Center" />
                                </asp:BoundField>
                        ...
                        <asp:SqlDataSource ID="SDSEventos" runat="server" ConnectionString="<%$ ConnectionStrings:NCCSANConnectionString %>"
                            ...
                            SelectCommand="SELECT e.codigo, e.work_order, CONVERT(VARCHAR(10), e.fecha_ingreso, 105) AS fecha_ingreso, c.nombre AS nombre_centro, e.nombre_cliente, e.irc, e.nombre_componente, e.nombre_tipoequipo, e.modelo_equipo, DATEDIFF(DAY, e.fecha_ingreso, SYSDATETIME()) as dias_actividad, e.estado FROM Evento e, EventoPersona ep, Centro c WHERE (ep.codigo_evento=e.codigo) AND (ep.rut_persona=@rut_persona) AND (ep.tipo=@tipo_persona) AND (c.id=e.id_centro) AND (CONVERT(VARCHAR(10), e.fecha_ingreso, 105) LIKE '%' + @fecha + '%') AND ((e.codigo LIKE '%' + @codigowo + '%') OR  (e.work_order LIKE '%' + @codigowo + '%')) AND (e.nombre_cliente LIKE '%' + @nombre_cliente + '%') AND ((e.estado LIKE '%' + @filtroEstado + '%') OR (e.estado IN (@estado1, @estado2, @estado3))) ORDER BY e.fecha_ingreso DESC">
                            <SelectParameters>
                                ...
                                <asp:ControlParameter ControlID="ddlCliente" ConvertEmptyStringToNull="False" 
                                    Name="nombre_cliente" PropertyName="SelectedValue" />
