<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="ListarEventos.aspx.cs" Inherits="NCCSAN.Registros.ListarEventos" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:UpdatePanel ID="uPanel" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            ...
            <asp:Panel ID="pnFiltro" runat="server">
                <table align="center" class="tableControls" style="width: 100%">
                    ...
                    <tr>
                        <td class="tdUnique" colspan="2" align="left">
                            <asp:CheckBox ID="chbCreadorEvento" runat="server" AutoPostBack="True" Checked="False"
                            Text="Registrado por" oncheckedchanged="chbCreadorEvento_CheckedChanged" />
                            &nbsp;&nbsp;&nbsp;
                            <asp:DropDownList ID="ddlCreadorEvento" runat="server" AutoPostBack="True" Enabled="False">
                            </asp:DropDownList>
                        </td>
                        <td class="tdUnique" colspan="4" align="center">
                            <asp:HiddenField ID="hfFiltroEstado" runat="server" />
                            <asp:CheckBox ID="chbFiltroEstado" runat="server" AutoPostBack="True" Checked="False"
                                OnCheckedChanged="chbFiltroEstado_CheckedChanged" Text="Filtrar estado" />
                            &nbsp;&nbsp;&nbsp;
                    ...
                    <td class="tdUnique">
                        <asp:GridView ID="gvEventos" runat="server" AutoGenerateColumns="False" DataKeyNames="codigo"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="nombre_creador" HeaderText="Creador" 
                                    SortExpression="nombre_creador" />
		...
		e.codigo, 
		e.work_order, 
		CONVERT(VARCHAR(10), e.fecha_ingreso, 105) AS fecha_ingreso, 
		e.nombre_cliente, 
		e.nombre_fuente, 
		e.irc, 
		e.nombre_componente, 
		DATEDIFF(DAY, e.fecha_ingreso, SYSDATETIME()) as dias_actividad, 
		e.estado, 
...
WHERE 
		...
		(ep.codigo_evento=e.codigo) AND 
		(p.rut=ep.rut_persona) AND 
		(ep.tipo=@tipo) AND 
		(e.nombre_cliente LIKE @nombre_cliente + '%') AND 
		(
			((LEN(@nombre_area) &gt; 0) AND (e.nombre_area LIKE @nombre_area + '%')) OR
			(LEN(@nombre_area) &lt; 1)
		) AND
		(e.nombre_fuente LIKE @nombre_fuente + '%') AND 
		(CONVERT(VARCHAR(10), e.fecha_ingreso, 105) LIKE '%' + @fecha + '%') AND 
		(
			(e.codigo LIKE '%' + @codigowo + '%') OR 
			(e.work_order LIKE '%' + @codigowo + '%')
		) AND 
		(p.rut LIKE @rut_creador + '%') AND 
		(
			(e.estado LIKE '%' + @filtroEstado + '%') OR 
			(e.estado IN (@estado1, @estado2, @estado3))
		) 
ORDER BY 
		e.fecha_ingreso DESC">
                            <SelectParameters>
                                <asp:SessionParameter Name="id_centro" SessionField="id_centro" />
                                <asp:Parameter DefaultValue="Creador" Name="tipo" />
                                ...
                                <asp:ControlParameter ControlID="ddlCreadorEvento" 
                                    ConvertEmptyStringToNull="False" Name="rut_creador" 
